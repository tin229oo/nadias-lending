...
    qs('.nav-toggle')?.addEventListener('click',()=>{
    ...
    // --- Login and Register Wiring (The forms that weren't working) ---
    // Register page
...
    const contact = qs('#contactForm'); 
    if(contact){
        contact.addEventListener('submit',e=>{
            e.preventDefault();
            alert('Message sent — for demo it uses local simulation.');
            contact.reset();
        })
    }
...
        e.preventDefault();
        const email = qs('#subEmail', this)?.value.trim();
